...
public static class Test_Query
{
    ...
    public static void Test_Query_arg_count_5()
    {
        ...
        // --- force one time allocations
        var  query = store.Query<Position, Rotation, Scale3, WorldTransform, EntityName>();
        int chunkCount = 0;
        foreach (var chunk in query.Chunks) {
            if (chunkCount++ == 0) {
                Mem.AreEqual(1, chunk.Length);
    ...
    private static void AssertChunkExtensions(ArchetypeQuery<Position, Rotation, Scale3, WorldTransform, EntityName> query) {
